...
public class PermissionEmployeeDaoImpl extends
		GenericDaoImpl<PermissionEmployee, Long> implements
		PermissionEmployeeDao {
	...
	public PermissionEmployee buscaPorMatricula(String matricula) {
		Query query = this.entityManager
				.createNamedQuery("PermissionEmployee.buscaPorMatricula");
		query.setParameter("matricula", matricula);

		@SuppressWarnings("unchecked")
		List<PermissionEmployee> permissions = query.getResultList();

		PermissionEmployee resultado = permissions.isEmpty() ? null
				: permissions.get(0);
